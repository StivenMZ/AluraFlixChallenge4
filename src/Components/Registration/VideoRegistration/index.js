...
const RegistroVideo = () => {
    ...
    const categorias = ["Entretenimiento", "Videojuegos", "Educación", "Ciencia"]
...
const updateValue = (value, setValue) => {
  ...
  return (
    <DefaultPage>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <TextField
          ...
          value={enlaceVideo}
          onChange={(e)=>{
            updateValue(e.target.value, setEnlaceVideo)
        }}
        borderColor={"#2A7AE4"}
        />
        <TextField
          ...
          value={enlaceImagen}
          onChange={(e)=>{
            updateValue(e.target.value, setEnlaceImagen)
        }}
        borderColor={"#2A7AE4"}
        />
       <DivOptions>
          <Options id="categoria" value={categoria} onChange={selectCategoria}>
            {categorias.map((cate, index)=>{
              return <ElementOpt key={index} >{cate}</ElementOpt> 
            })}
    ...
    <DivBotones >
        <DivSubBtn>
        <Button borderColor={"#2A7AE4"} textColor={"white"} text={"Registrar"} backGround={"#2A7AE4"}></Button>
